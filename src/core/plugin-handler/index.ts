...
import type { AdapterHandlerOptions, AdapterInfo } from '@/core/plugin-handler/types';
...
class AdapterHandler {
  ...
  private async execCommand(cmd: string, modules: string[]): Promise<string> {
    return new Promise((resolve: any, reject: any) => {
      let args: string[] = [cmd].concat(cmd !== 'uninstall' && cmd !== 'link' ? modules.map(m => `${m}@latest`) : modules);
      if (cmd !== 'link') {
